...
2. **Execute Installer**:
   - **Windows**: Run `powershell.exe -ExecutionPolicy Bypass -File scripts/install.ps1`.
   - **Unix**: Run `bash scripts/install.sh`.
3. **Finalize**: Notify the user that the update is complete and they should restart any active sessions if necessary.
